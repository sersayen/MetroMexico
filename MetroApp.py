...
class MetroApp:
    def __init__(self, root):
        ...
        self.afluencia_status_label = tk.Label(frame_controles, text="Afluencia:", bg="lightyellow")
        self.afluencia_status_label.grid(row=3, column=0, columnspan=2, pady=5, sticky="ew")
        ...
        self.dificil_acceso_label = tk.Label(frame_controles, text="Estaciones de difícil acceso: {Chapultepec, Insurgentes, Etiopia,\n Eugenia, Division del Norte, Coyoacan, Lazaro Cardenas}", bg="lightyellow")
        self.dificil_acceso_label.grid(row=5, column=0, columnspan=2, pady=5, sticky="ew")
    ...
    def calcular_ruta(self):
        ...
        if inicio not in nodos_validos:
            self.resultado_label.config(text=f"La estacion de inicio '{inicio}' no es una estación válida")
            self.tiempo_label.config(text="")
            return 
        if destino not in nodos_validos:
            self.resultado_label.config(text=f"La estacion destino '{destino}' no es una estación válida")
            self.tiempo_label.config(text="")
            return 
